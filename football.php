<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Document</title>
  <link href="https://fonts.googleapis.com/css2?family=Handlee&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" integrity="sha512-iecdLmaskl7CVkqkXNQ/ZH/XLlvWZOJyj7Yy7tcenmpD1ypASozpmT/E0iPtmFIB46ZmdtAc9eNBvH0H/ZpiBw==" crossorigin="anonymous" referrerpolicy="no-referrer" />
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
  
  <link rel="stylesheet" href="home.css">
  <link rel="stylesheet" href="football.css">
  <style>
           .ok{
            /* border: 1px solid white; */
            
            background-color: rgba(195, 220, 224,0.3);
           }
  </style>
</head>

<body>
    <div class="container">
    <header>
        <nav class="navbar navbar-expand-lg">
            <a class="navbar-brand text-white">ASH98 Sports Corner</a>

            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent">
              <!-- <span class="navbar-toggler-icon"></span> -->
              <span><i class="fa-solid fa-bars-progress navbar-icon"></i></span> 
            </button>
          
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
              <ul class="navbar-nav mr-auto">
                <li class="nav-item active">
                  <a class="nav-link text-center  text-md-left" href="home2.php">Home</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link text-center  text-md-left" href="#">About me</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link text-center  text-md-left" href="cricket.php">CRICKET</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link text-center  text-md-left" href="football.php">FOOTBALL</a>
                </li>
                 
                <li class="nav-item dropdown">
                  <a class="nav-link dropdown-toggle text-center  text-md-left" href="#" role="button" data-bs-toggle="dropdown">ASH98</a>
                  <ul class="dropdown-menu">
                    <li><a class="dropdown-item text-center  text-md-left" href="#">ASH About</a></li>
                    <li><a class="dropdown-item text-center  text-md-left" href="#">ASH</a></li>
                    <li><a class="dropdown-item text-center  text-md-left" href="#">ASH</a></li>
                  </ul>
                </li>
                
        
            </div>
          </nav>

        
      </header>
           
         <div class="row">
         
       <div class="col-lg-3 col-sm-6 col-md-4  ">
        <div class="card text-center  content m-2 " >
          <img class="card-img-top im" src="football/bd.jpg"  alt="Card image cap">
          <div class="card-body">
            <h3>Jersy</h3>
            <p>Bangladesh Home kit</p> 
            <h4>$75.00</h4>
            <h6>ID: 1</h6>
            <a href="card.php" class="btn btn-primary">Add To Card</a>
          </div>
        </div>
       </div>
       <div class="col-lg-3 col-sm-6 col-md-4  ">
        <div class="card text-center content  m-2 " >
          <img class="card-img-top im" src="football/b.jpeg" alt="Card image cap">
          <div class="card-body">
            <h3>Jersy</h3>
            <p>Brazil Home kit</p> 
            <h4>$75.00</h4>
            <h6>ID: 2</h6>
            <a href="card.php" class="btn btn-primary">Add To Card</a>
          </div>
        </div>
       </div>
       <div class="col-lg-3 col-sm-6 col-md-4 ">
        <div class="card text-center content  m-2 " >
          <img class="card-img-top im" src="football/e.jpeg" alt="Card image cap">
          <div class="card-body">
            <h3>Jersy</h3>
            <p>England Home kit</p> 
            <h4>$75.00</h4>
            <h6>ID: 3</h6>
            <a href="card.php" class="btn btn-primary">Add To Card</a>
          </div>
        </div>
       </div>
       <div class="col-lg-3 col-sm-6 col-md-4  ">
        <div class="card text-center content  m-2 " >
          <img class="card-img-top im" src="football/it.jpg" alt="Card image cap">
          <div class="card-body">
            <h3>Jersy</h3>
            <p>Itali Home kit</p> 
            <h4>$75.00</h4>
            <h6>ID: 4</h6>
            <a href="card.php" class="btn btn-primary">Add To Card</a>
          </div>
        </div>
       </div>
       <div class="col-lg-3 col-sm-6 col-md-4 m-2 ">
        <div class="card text-center content m-2 " >
          <img class="card-img-top im" src="football/fr.jpeg" alt="Card image cap">
          <div class="card-body">
            <h3>Jersy</h3>
            <p>France Home kit</p> 
            <h4>$75.00</h4>
            <h6>ID: 5</h6>
            <a href="card.php" class="btn btn-primary">Add To Card</a>
          </div>
        </div>
       </div>
       <div class="col-lg-3 col-sm-6 col-md-4  ">
        <div class="card text-center content  m-2 " >
          <img class="card-img-top im" src="football/R (1).jpeg" alt="Card image cap">
          <div class="card-body">
            <h3>Jersy</h3>
            <p>Argentina Home kit</p> 
            <h4>$75.00</h4>
            <h6>ID: 123001</h6>
            <a href="card.php" class="btn btn-primary">Add To Card</a>
          </div>
        </div>
       </div>

         </div>
            
           

    </div>
    
<script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.2/dist/umd/popper.min.js" integrity="sha384-IQsoLXl5PILFhosVNubq5LC7Qb9DXgDA9i+tQ8Zj3iwWAwPtgFTxbJ8NT4GN1R8p" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.min.js" integrity="sha384-cVKIPhGWiC2Al4u+LWgxfKTRIcfu0JTxR+EQDz/bgldoEyl4H0zUF0QKbrJ0EcQF" crossorigin="anonymous"></script>
</body>
</html>